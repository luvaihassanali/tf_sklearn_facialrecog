...
def plotImagePatches(ax, index, image, patches):
    ...
    fig.suptitle("Top 4 patches - greyscale input")
    ax[index][0].imshow(image.reshape(50,50), interpolation="nearest", cmap='gray')
    ...
    for i in range(4):
        ...
        ax[index][i+1].imshow(plot_list[i][1], interpolation="nearest", cmap='gray')
    for i in range(7):
...
images = images.reshape(1288,50,50,1)
...
X = tf.placeholder("float", [None, 50, 50, 1], name="X")
Y = tf.placeholder("float", [None, 7], name="Y")

#init weights
w = init_weights([1, 1, 1, 50], "w")       
...
with tf.Session() as sess:
    print("\nCNN LFW Classification - Greyscale Input\n")
    sess.run(init)
    ...
    for i in range(20): #epochs
        ...
        acc = np.mean(np.argmax(teY, axis=1) == sess.run(predict_op, feed_dict={X: teX, p_keep_conv: 1.0, p_keep_hidden: 1.0}))
        print(i+1,"=",acc)
    ...
    for i in range(len(imgs)):
        patches = sess.run(conv1,feed_dict={X:imgs[i].reshape(1,50,50,1), p_keep_conv: 1.0, p_keep_hidden: 1.0})
